...
%--------------------------------------------------------------------------------------------------%
        function ampac = run_Ampac(obj, phi, suppress_output)
            ...
            if (~data_exist)
                ...
                if (~suppress_output)
                    disp(['Running AMPAC calculation on ',obj.ampacdatapath, hash]);
                end
                ...
                if (ampac.ampac_succeed)
                    if (obj.params.doIndo)
                        ...
                        if (~data_exist)
                            ...
                            tmp_ampac_path = obj.ampacdatapath;
                            if (obj.ampacdatapath(end-1) == ':')
                                tmp_ampac_path = tmp_ampac_path(1:end-1);
                            end
                            tmp_indo_path = obj.indodatapath;
                            if (obj.indodatapath(end-1) == ':')
...
%                             end
                ...
                % pause(0.02); % Hard drive delay catch-up...
                if (exist([obj.indodatapath, indo_out.hash, '.ido'], 'file'))
                    delete([obj.indodatapath, indo_out.hash, '.ido']);
                end
...
%                     mytic = tic();
            ...
            for i = 1:obj.bigO
                if (~isempty(obj.data(i).Ehf))
                    obj.data(i).Ehf = (obj.data(i).Ehf - mindata) * (1/23.05);
                end
            end
...
%--------------------------------------------------------------------------------------------------%
        function successful = update_paths(obj, newampac, newindo, save_mat_file)
            if (~isempty(newampac))
                ...
                if (exist([newampac, 'catalog.mat'], 'file'))
                    ...
                    if (~ismember(ampachashes, alib.keys()))
                        throw(MException('EnergyCalcExp:update_paths:NewAmpacLibMissingData',...
            ...
            if (~isempty(newindo))
                ...
                % pause(0.02); % Hard drive delay catch-up...
                if (exist([newindo, 'catalog.mat'], 'file'))
                    ...
                    fn = {files(1:end).name};
                    fn = cellfun(@(x)regexpi(x, '^(([A-Fa-f0-9]){32})\.mat', 'tokens'), fn, 'UniformOutput', false);
                    fn = {fn{~(cellfun(@(x)isempty(x), fn))}};
                    fn = cat(2, fn{1:end});
                    fn = [fn{:}];
                    ...
                    if (length(fn) >= numel(obj.data))
                        ...
                        if (~isempty(nonmember))
                            throw(MException('EnergyCalcExp:update_paths:NewIndoLibMissingData',...
                                'EnergyCalcExp: New INDO library does not contain necessary data.'));
                        end
                    else
                        throw(MException('EnergyCalcExp:update_paths:NewIndoLibMissingData',...
                ...
                if (obj.params.doIndo)
                    ...
                    while (isempty(indo_out.hash))
                        if (~isempty(obj.indo_lib))
                            ...
                            % pause(0.5); % Hard drive delay catch-up...
                            if (exist([obj.indodatapath, hash, '.mat'], 'file'))
                                S = load([obj.indodatapath, hash, '.mat'], 'key');
                                if (~isequal(S.key, key))
                                    hash = DataHash(hash);
...
%                         end
            ...
            if (obj.fail_iterator == 1 || ~strcmp(obj.fail_bucket(1).type, 'ampac'))
                obj.data(1).Ehf = obj.data(1).Ehf * (1/23.05);
            end
            if (obj.fail_iterator ~= 1)
                warning('off','backtrace');
