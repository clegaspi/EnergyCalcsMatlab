...
if (exist(['s:\NoAngle\',mols{imol,1},'-0VA.mat'], 'file'))
    S = load(['s:\NoAngle\',mols{imol,1},'-0VA.mat']);
    myexp = S.obj;
    if (exist(['o:\', myexp.data(1).indo_hash, '-dm.bin'],'file'))
...
elseif (dblcmp(field(1),0))
    ...
    myexp = EnergyCalcExp(myece,...
        ...
        mols{imol,3},...
        ['s:\NoAngle\',mols{imol,1},'-',num2str(field(k)),'VA.mat'],...
        'o:\',...
        'p:\',...
        false,...
...
%     end
    ...
    if (exist(['s:\NoAngle\',mols{imol,1},'-',num2str(field(pk)),'VA.mat'], 'file'))
        continue;
    else
        ...
        pmyexp = EnergyCalcExp(myece,...
            ['s:\NoAngle\',mols{imol,1},'.dat'],...
            mols{imol,3},...
            ['s:\NoAngle\',mols{imol,1},'-',num2str(field(pk)),'VA.mat'],...
            'o:\',...
            'p:\',...
            false,...
...
for fstrn = fields_to_opt_struct;
    ...
    S = load(['s:\NoAngle\',mols{imol,1},'-',num2str(fstrn),'VA.mat']);
    myexp = S.obj;
