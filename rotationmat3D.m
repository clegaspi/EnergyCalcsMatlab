...
R = nan(3);
%fill
R(1,1) =  u2 + (v2 + w2)*c(1,1);
R(1,2) = u*v*(1-c(1,2)) - w*s(1,2);
R(1,3) = u*w*(1-c(1,3)) + v*s(1,3);
R(2,1) = u*v*(1-c(2,1)) + w*s(2,1);
R(2,2) = v2 + (u2+w2)*c(2,2);
R(2,3) = v*w*(1-c(2,3)) - u*s(2,3);
R(3,1) = u*w*(1-c(3,1)) - v*s(3,1);
R(3,2) = v*w*(1-c(3,2))+u*s(3,2);
R(3,3) = w2 + (u2+v2)*c(3,3);
end